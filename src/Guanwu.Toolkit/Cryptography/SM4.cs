
using System;
using System.Collections.Generic;

namespace Guanwu.Toolkit.Cryptography
{
    internal class Sm4Context
    {
        public bool IsPadding { get; set; } = true;
        public int[] SubKeys { get; set; } = new int[32];
    }

    internal class Sm4
    {
        public static byte[] SboxTable = new byte[] {
            
        };

        public static uint[] FK = {
            0xa3b1bac6,0x56aa3350,0x677d9197,0xb27022dc
        };

        public static uint[] CK = {
            0x00070e15,0x1c232a31,0x383f464d,0x545b6269,
            0x70777e85,0x8c939aa1,0xa8afb6bd,0xc4cbd2d9,
            0xe0e7eef5,0xfc030a11,0x181f262d,0x343b4249,
            0x50575e65,0x6c737a81,0x888f969d,0xa4abb2b9,
            0xc0c7ced5,0xdce3eaf1,0xf8ff060d,0x141b2229,
            0x30373e45,0x4c535a61,0x686f767d,0x848b9299,
            0xa0a7aeb5,0xbcc3cad1,0xd8dfe6ed,0xf4fb0209,
            0x10171e25,0x2c333a41,0x484f565d,0x646b7279
        };

        private static int GET_ULONG_BE(byte[] b, int i)
        {
            return (int)(
                (uint)(b[i] & 0xff) << 24 |
                (uint)(b[i + 1] & 0xff) << 16 |
                (uint)(b[i + 2] & 0xff) << 8 |
                (uint)(b[i + 3] & 0xff) & 0xffffffff
            );
        }

        private static void PUT_ULONG_BE(int n, byte[] b, int i)
        {
            b[i] = (byte)(uint)(0xff & n >> 24);
            b[i + 1] = (byte)(uint)(0xff & n >> 16);
            b[i + 2] = (byte)(uint)(0xff & n >> 8);
            b[i + 3] = (byte)(uint)(0xff & n);
        }

        private static int ROTL(int x, int n)
        {
            return (int)(
                (uint)(x & 0xffffffff) << n |
                (uint)(x >> (32 - n))
            );
        }

        private static byte Sm4Sbox(byte inch)
        {
            var i = inch & 0xff;
            return SboxTable[i];
        }

        private static int Sm4Lt(int ka)
        {
            var a = new byte[4];
            var b = new byte[4];
            PUT_ULONG_BE(ka, a, 0);
            b[0] = Sm4Sbox(a[0]);
            b[1] = Sm4Sbox(a[1]);
            b[2] = Sm4Sbox(a[2]);
            b[3] = Sm4Sbox(a[3]);
            var bb = GET_ULONG_BE(b, 0);
            return (int)(
                bb ^ ROTL(bb, 2) ^ ROTL(bb, 10) ^ ROTL(bb, 18) ^ ROTL(bb, 24)
            );
        }

        private static int Sm4F(int x0, int x1, int x2, int x3, int rk)
        {
            return x0 ^ Sm4Lt(x1 ^ x2 ^ x3 ^ rk);
        }

        private static int Sm4CalciRk(int ka)
        {
            byte[] a = new byte[4];
            byte[] b = new byte[4];
            PUT_ULONG_BE(ka, a, 0);
            b[0] = Sm4Sbox(a[0]);
            b[1] = Sm4Sbox(a[1]);
            b[2] = Sm4Sbox(a[2]);
            b[3] = Sm4Sbox(a[3]);
            int bb = GET_ULONG_BE(b, 0);
            return (int)(
                bb ^ ROTL(bb, 13) ^ ROTL(bb, 23)
            );
        }

        private static void SetKey(int[] SK, byte[] key)
        {
            int[] MK = new int[4];
            MK[0] = GET_ULONG_BE(key, 0);
            MK[1] = GET_ULONG_BE(key, 4);
            MK[2] = GET_ULONG_BE(key, 8);
            MK[3] = GET_ULONG_BE(key, 12);

            int[] k = new int[36];
            k[0] = MK[0] ^ (int)FK[0];
            k[1] = MK[1] ^ (int)FK[1];
            k[2] = MK[2] ^ (int)FK[2];
            k[3] = MK[3] ^ (int)FK[3];

            for (int i = 0; i < 32; i++) {
                k[(i + 4)] = k[i] ^ Sm4CalciRk(k[(i + 1)] ^ k[(i + 2)] ^ k[(i + 3)] ^ (int)CK[i]);
                SK[i] = k[(i + 4)];
            }
        }

        public void SetKeyEnc(Sm4Context ctx, byte[] key)
        {
            SetKey(ctx.SubKeys, key);
        }

        private byte[] Padding(byte[] input)
        {
            if (input == null) return null;

            int p = 16 - input.Length % 16;
            byte[] ret = new byte[input.Length + p];
            Array.Copy(input, 0, ret, 0, input.Length);
            for (int i = 0; i < p; i++) {
                ret[input.Length + i] = (byte)p;
            }

            return ret;
        }

        private static void Sm4OneRound(int[] sk, byte[] input, byte[] output)
        {
            int[] ulbuf = new int[36];
            ulbuf[0] = GET_ULONG_BE(input, 0);
            ulbuf[1] = GET_ULONG_BE(input, 4);
            ulbuf[2] = GET_ULONG_BE(input, 8);
            ulbuf[3] = GET_ULONG_BE(input, 12);

            for (var i = 0; i < 32; i++) {
                ulbuf[(i + 4)] = Sm4F(ulbuf[i], ulbuf[(i + 1)], ulbuf[(i + 2)], ulbuf[(i + 3)], sk[i]);
            }

            PUT_ULONG_BE(ulbuf[35], output, 0);
            PUT_ULONG_BE(ulbuf[34], output, 4);
            PUT_ULONG_BE(ulbuf[33], output, 8);
            PUT_ULONG_BE(ulbuf[32], output, 12);
        }

        public byte[] EncryptCBC(Sm4Context ctx, byte[] iv, byte[] input)
        {
            if (ctx.IsPadding) input = Padding(input);

            int length = input.Length;
            byte[] bins = new byte[length];
            Array.Copy(input, 0, bins, 0, length);
            var bousList = new List<byte>();

            for (int j = 0; length > 0; length -= 16, j++) {
                byte[] inBytes = new byte[16];
                byte[] outBytes = new byte[16];
                byte[] out1 = new byte[16];

                Array.Copy(bins, j * 16, inBytes, 0, length > 16 ? 16 : length);
                for (int i = 0; i < 16; i++) {
                    outBytes[i] = ((byte)(inBytes[i] ^ iv[i]));
                }
                Sm4OneRound(ctx.SubKeys, outBytes, out1);
                Array.Copy(out1, 0, iv, 0, 16);
                for (int k = 0; k < 16; k++) {
                    bousList.Add(out1[k]);
                }
            }

            var output = bousList.ToArray();
            for (int i = 0; i < output.Length; i++) {
                if (output[i] < 0)
                    output[i] = (byte)(output[i] + 256);
            }

            return output;
        }
    }
}